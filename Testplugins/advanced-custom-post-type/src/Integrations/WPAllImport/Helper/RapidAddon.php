...
namespace ACPT\Integrations\WPAllImport\Helper;
...
/**
 * Class WPAllImportAddon
 * @package ACPT\Integrations\WPAllImport\Addon
 * @see https://github.com/soflyy/wp-all-import-example-addon/blob/master/rapid-addon.php
 */
class RapidAddon {
    ...
    <h3 class="handle" style="margin-top:0;"><span>'.$acf_group['title'].'</span></h3>
	    <div class="inside">');
	...
	function add_text($text = '', $is_html = false){
		...
		$count = is_array($this->fields) ? Arrays::count($this->fields) : 0;
	...
	function import_images( $slug, $title, $type = 'images' ){
		...
		if (Arrays::count($this->image_sections) > 1){
	...
	*/
	function admin_notice($notice_text = '', $conditions = array()) {
		...
		if ( ! $is_show_notice and ! empty($conditions['plugins']) ){
			...
			if ($requires_counter != Arrays::count($conditions['plugins'])){
				$is_show_notice = true;
			}
