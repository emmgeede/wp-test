...
use ACPT\Core\Traits\PermissionTrait;
...
class MetaFieldModel extends AbstractModel implements \JsonSerializable
{
    ...
    const AUDIO_MULTI_TYPE = 'AudioMulti';
	const BARCODE_TYPE = 'Barcode';
	...
	const IMAGE_SLIDER_TYPE = 'ImageSlider';
     ...
     * @var bool|null
     */
    private ?bool $isAWooCommerceProductAttribute = false;
    ...
    /**
     * This function converts the meta field type to
     * the corresponding form field type
     *
     * @return string
     */
	public function resolveFieldTypeForForms()
    {
        if($this->type === MetaFieldModel::ADDRESS_MULTI_TYPE){
            return MetaFieldModel::ADDRESS_TYPE;
        }

        if($this->type === MetaFieldModel::EDITOR_TYPE){
            return MetaFieldModel::TEXTAREA_TYPE;
        }

        if($this->type === MetaFieldModel::SELECT_MULTI_TYPE){
            return MetaFieldModel::SELECT_TYPE;
        }

        if($this->type === MetaFieldModel::USER_MULTI_TYPE){
            return MetaFieldModel::USER_TYPE;
        }

        if($this->type === MetaFieldModel::TERM_OBJECT_MULTI_TYPE){
            return MetaFieldModel::TERM_OBJECT_TYPE;
        }

        if($this->type === MetaFieldModel::POST_OBJECT_MULTI_TYPE){
	...
	{
		return Arrays::count($this->visibilityConditions) > 0;
	}
	...
	public function getDbName()
	{
		...
		if(!$this->isAWooCommerceProductAttribute and $this->getBelongsToLabel() === MetaTypes::OPTION_PAGE and $this->getFindLabel() !== null){
     ...
     * @return bool
     */
    public function isAWooCommerceProductAttribute(): bool
    {
        return $this->isAWooCommerceProductAttribute ?? false;
    }

    /**
     * @param bool|null $isAWooCommerceProductAttribute
     */
    public function setIsAWooCommerceProductAttribute( ?bool $isAWooCommerceProductAttribute ): void
    {
        $this->isAWooCommerceProductAttribute = $isAWooCommerceProductAttribute;
    }
     ...
     * @return bool
     */
	public function isForFormBuilder()
    {
        $allowed = [
            self::ADDRESS_TYPE,
            self::ADDRESS_MULTI_TYPE,
            self::AUDIO_TYPE,
            self::AUDIO_MULTI_TYPE,
            self::BARCODE_TYPE,
            self::CHECKBOX_TYPE,
            self::CLONE_TYPE,
            self::COLOR_TYPE,
	...
	public function arrayRepresentation(string $format = 'full'): array
	{
		...
		return [
			...
			'isRequired' => (bool)$this->isRequired(),
			'isAWooCommerceProductAttribute' => (bool)$this->isAWooCommerceProductAttribute(),
	...
	public static function validationRules(): array
	{
		return [
			...
			'isAWooCommerceProductAttribute' => [
				'required' => false,
				'type' => 'boolean',
			],
			'required' => [
    ...
    /**
     * This function adds BE validation against min and max attributes
     *
     * @param null $rawValue
     * @throws \Exception
     */
    public function validateAgainstMaxAndMin($rawValue = null)
    {
        if($rawValue === null){
            return;
        }
        ...
        if(!empty($min) and !empty($rawValue)){
            if($this->isNumeric()){
                if($rawValue < $min){
                    throw new \Exception('There was an error during saving data: ' . $rawValue . ' is less than ' . $min);
                }
            } elseif($this->isTextual()){
                if(strlen($rawValue) < $min){
                    throw new \Exception('There was an error during saving data: ' . $rawValue . ' string length is less than ' . $min);
                }
            }
        }

        if(!empty($max) and !empty($rawValue)){
            if($this->isNumeric()){
                if($rawValue > $max){
                    throw new \Exception('There was an error during saving data: ' . $rawValue . ' is greater than ' . $max);
                }
            } elseif($this->isTextual()){
                if(strlen($rawValue) > $max){
                    throw new \Exception('There was an error during saving data: ' . $rawValue . ' string length is greater than ' . $max);
                }
            }
        }
    }
    ...
    /**
     * Validates the file size against the specified minimum and maximum limits.
     *
     * @param int $fileSize The size of the file in bytes.
     * @return void
     * @throws \Exception If the file size is smaller than the minimum size or larger than the maximum size.
     */
    public function validateFileSize($fileSize): void
    {
        if(empty($fileSize)){
            return;
        }
        ...
        if (!empty($minSize) and $fileSize < $minSize * 1024 * 1024) {
            throw new \Exception("Error: File is less than $minSize Mb.");
        }

        if (!empty($maxSize) and $fileSize > $maxSize * 1024 * 1024) {
            throw new \Exception("Error: File is greater than $maxSize Mb.");
        }
    }

    /**
     * Validates the dimensions of an image against the configured minimum and maximum width and height limits.
     *
     * @param string $filePath The temporary file path of the image to validate.
     * @return void
     *
     * @throws \Exception If the image exceeds maximum width or height, is smaller than the minimum width or height, or is not a valid image file.
     */
    public function validateImageDimension($filePath): void
    {
        if (empty($filePath)) {
            return;
        }
        ...
        // Discard not image files
        if (!Strings::startsWith($mimeType, 'image/')) {
            return;
        }
        ...
        if ($imageInfo) {
            ...
            if (!empty($maxWidth) and $width > $maxWidth) {
                throw new \Exception( "Error: Image is greater than $maxWidth px wide.");
            }

            if (!empty($maxHeight) and $height > $maxHeight) {
                throw new \Exception( "Error: Image is greater than $maxHeight px tall.");
            }

            if (!empty($minWidth) and $width < $minWidth) {
                throw new \Exception( "Error: Image is less than $minWidth px wide.");
            }

            if (!empty($minHeight) and $height < $minHeight) {
                throw new \Exception( "Error: Image is less than $minHeight px tall.");
            }

        } else {
            throw new \Exception( "Error: File is not a valid image.");
        }
    }

    /**
     * Validates the MIME type of a given file against a set of allowed MIME types.
     *
     * @param string $filePath The temporary file path to validate.
     * @return void
     * @throws \Exception If the file's MIME type is not allowed.
     */
    public function validateFileMimeType($filePath): void
    {
        if(empty($filePath)){
            return;
        }

        $allowedMimeTypes = $this->getAdvancedOption('accepts') ?? null;

        if(empty($allowedMimeTypes)){
            return;
        }

        if(is_serialized($allowedMimeTypes)){
            $allowedMimeTypes = unserialize($allowedMimeTypes);
        }
        ...
        $detectedType  = (string) @mime_content_type($filePath);

        if (!in_array($detectedType, $allowedMimeTypes)) {
            throw new \Exception("Error: Invalid file format. Detected: $detectedType");
        }
    }
}