...
class TableFieldGenerator extends AbstractGenerator
{
     ...
     */
	public function __construct($json, $sortable = null, $pagination = null)
	{
    ...
    /**
     * Enqueue the js assets
     *
     * @param $layout
     */
	private function enqueueAssets($layout)
    {
        // table_sortable
        if($layout === 'horizontal' and $this->sortable){
            wp_register_script('acpt-table-sortable-js',  plugins_url( ACPT_DEV_MODE ? 'advanced-custom-post-type/assets/static/js/table_sortable.js' : 'advanced-custom-post-type/assets/static/js/table_sortable.min.js'), [], ACPT_PLUGIN_VERSION );
            wp_enqueue_script('acpt-table-sortable-js');
        }

        // pagination
        if($this->pagination){
            wp_register_script('acpt-table-pagination-js',  plugins_url( ACPT_DEV_MODE ? 'advanced-custom-post-type/assets/static/js/table_pagination.js' : 'advanced-custom-post-type/assets/static/js/table_pagination.min.js'), [], ACPT_PLUGIN_VERSION );
            wp_enqueue_script('acpt-table-pagination-js');
        }

        wp_enqueue_style( 'acpt-table-css', plugins_url( ACPT_DEV_MODE ? 'advanced-custom-post-type/assets/static/css/table.css' : 'advanced-custom-post-type/assets/static/css/table.min.css'), [], ACPT_PLUGIN_VERSION, 'all');
    }
	...
	private function createVerticalTable($columns, $rows, $data, $css, $footer = false)
	{
        ...
        if($this->pagination !== null){
            $css .= " acpt-table-with-pagination";
            $jsonData = $this->formatJsonData($data, $maths, 'vertical', false);
        }

		$table = '<table class="acpt-table '.$css.'" data-pagination="'.$this->pagination.'" data-value=\''.$jsonData.'\'>';
		$rowIndex = 0;
		$index = 0;
        ...
        if(empty($this->pagination)){
            for ($i = 0; $i < $rows; $i++) {
                ...
                for ($k = 0; $k < $columns; $k++) {
                    ...
                    $table .=  do_shortcode($maths->eval($data[$index][$k]['value']));
	...
	private function createHorizontalTable($columns, $rows, $data, $css, $header = true, $footer = false)
	{
        ...
        if($this->pagination !== null){
            $css .= " acpt-table-with-pagination";
            $jsonData = $this->formatJsonData($data, $maths, 'horizontal', $header);
        }
     ...
     * @return false|string
     */
	private function formatJsonData($data, Maths $maths, $layout, $header)
    {
        // remove header for horizontal tables
        if($layout === "horizontal" and $header === true){
            unset($data[0]);
        ...
        foreach ($data as $rowIndex => $row){
            ...
            foreach ($row as $k => $cell){
                ...
                $formattedStyle = str_replace('style="',"", $this->applyCSStoCell($rowIndex, $k));
                $formattedStyle = substr($formattedStyle, 0, -1);

                $formattedRow[] = [
                    'value' => do_shortcode($maths->eval($cell['value'])),
                    'style' => $formattedStyle,
                    'tag' => ($layout === "vertical" and $k === 0) ? "th" : "td"
                ];
            }
     ...
     * @return string|null
     */
    public static function googleSheetsKey()
    {
        try {
            return Settings::get(SettingsModel::GOOGLE_SHEETS_API_KEY);
