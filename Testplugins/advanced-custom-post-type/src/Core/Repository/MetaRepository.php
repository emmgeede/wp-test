...
class MetaRepository extends AbstractRepository
{
	...
	public static function count()
	{
		$baseQuery = "
            SELECT 
                count(id) as count
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_GROUP)."`
            ";
	...
	public static function deleteBelongs($belongsTo, $find)
	{
		$sql = "
			SELECT b.id FROM 
			 `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_BELONG)."` b
			WHERE b.belongs =  %s
			AND b.find =  %s
		;";

		$belongs = ACPT_DB::getResults($sql, [
			$belongsTo,
			$find
		]);
		...
		foreach ($belongs as $belong){
			$sql = "
	            DELETE
                FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_BELONG)."`
                WHERE id = %s
            ";

			ACPT_DB::executeQueryOrThrowException($sql, [$belong->id]);

			$sql = "
	            DELETE
                FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_GROUP_BELONG)."`
                WHERE belong_id = %s
            ";
	...
	public static function deleteMetaGroup(string $id)
	{
		...
		try {
            ...
            $belongQuery = "
		        SELECT 
		        	b.id
	            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_BELONG)."` b
	            LEFT JOIN  `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_GROUP_BELONG)."` bb on bb.belong_id = b.id
	            WHERE bb.group_id = %s
	            GROUP BY b.id
	            ORDER BY b.sort
		    ";

            $belongs = ACPT_DB::getResults($belongQuery, [$id]);
			...
			$sql = "
				SELECT b.id FROM 
				 `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b
				where b.group_id =  %s
			;";

			$boxes = ACPT_DB::getResults($sql, [
				$id
			]);
    ...
    public static function deleteMetaBox(string $boxId)
    {
        $sql = "
            DELETE
                FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."`
                WHERE id = %s
            ";

		$sql2 = "
            DELETE
                FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."`
                WHERE meta_box_id = %s
            ";

		$sql3 = "
            DELETE
                FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_OPTION)."`
                WHERE meta_box_id = %s
            ";

		$sql4 = "
            DELETE
                FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_VISIBILITY)."`
                WHERE meta_box_id = %s
            ";

		$sql5 = "
            DELETE
                FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_RELATION)."`
                WHERE meta_box_id = %s
            ";

		$sql6 = "
            DELETE
                FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_ADVANCED_OPTION)."`
                WHERE meta_box_id = %s
            ";

		try {
			ACPT_DB::executeQueryOrThrowException($sql, [$boxId]);
			ACPT_DB::executeQueryOrThrowException($sql2, [$boxId]);
			ACPT_DB::executeQueryOrThrowException($sql3, [$boxId]);
			ACPT_DB::executeQueryOrThrowException($sql4, [$boxId]);
			ACPT_DB::executeQueryOrThrowException($sql5, [$boxId]);
			ACPT_DB::executeQueryOrThrowException($sql6, [$boxId]);
		} catch (\Exception $exception){
            do_action("acpt/error", $exception);
			ACPT_DB::rollbackTransaction();
			throw new \Exception($exception->getMessage());
	    ...
	    $sql = "
	        SELECT v.id FROM 
				`".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_VALIDATION_RULE)."` v
				join `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_VALIDATION_RULE_FIELD_PIVOT)."` vp on vp.rule_id=v.id
        ...
        $rules = ACPT_DB::getResults($sql, [
	        $boxId
        ]);

        foreach ($rules as $rule){
	        ACPT_DB::executeQueryOrThrowException("DELETE FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_VALIDATION_RULE)."` WHERE id = %s", [$rule->id]);
	        ACPT_DB::executeQueryOrThrowException("DELETE FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_VALIDATION_RULE_FIELD_PIVOT)."` WHERE rule_id = %s", [$rule->id]);
		}
	...
	public static function deleteMetaBlock(string $blockId)
	{
		...
		$sql = "
            DELETE
                FROM `".ACPT_DB::TABLE_META_BLOCK."`
                WHERE id = %s
            ";

		try {
			ACPT_DB::executeQueryOrThrowException($sql, [$blockId]);
			...
			$blocks = ACPT_DB::getResults($sql, [ $blockId ]);
	...
	public static function deleteMetaField(string $fieldId)
    {
		...
		try {
			ACPT_DB::executeQueryOrThrowException($sql, [$fieldId]);
			ACPT_DB::executeQueryOrThrowException($sql2, [$fieldId]);
			ACPT_DB::executeQueryOrThrowException($sql3, [$fieldId]);
			ACPT_DB::executeQueryOrThrowException($sql4, [$fieldId]);
			ACPT_DB::executeQueryOrThrowException($sql5, [$fieldId]);
        } catch (\Exception $exception){
            do_action("acpt/error", $exception);
            ACPT_DB::rollbackTransaction();
            throw new \Exception($exception->getMessage());
	...
	public static function existsMetaBoxField(array $args)
	{
	    ...
	    $baseQuery = "
            SELECT 
                f.id
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` f
    ...
    public static function metaFieldsFlatArray($excludeId)
    {
	    $guery = "
            ...
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` f
            LEFT JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b ON b.id = f.meta_box_id
            WHERE f.id != %s
	    ";

	    return ACPT_DB::getResults($guery, [$excludeId]);
    }
	...
	public static function get(array $args): array
    {
		...
		$postIdFromContext = isset($args['postId']) ? $args['postId'] : null; // post ID injected from the context
		...
		return array_values(array_filter($results, function (MetaGroupModel $groupModel) use ($belongsTo, $find, $postIdFromContext) {

            if($belongsTo){
                return $groupModel->isVisible([
                    'belongsTo' => $belongsTo,
                    'find' => $find,
                    'post_id' => $postIdFromContext,
                ]);
            }
     ...
     * @param null $find
     * @param bool $clonedFields
     * @param bool $formBuilder
     *
     * @return MetaGroupModel[]
     * @throws \Exception
     */
    ...
    )
    {
        ...
        if($belongsTo){
            $cachedId .= "_".$belongsTo;
        }

        if($groupName){
            $cachedId .= "_".$groupName;
        }
        ...
        if($excludeField){
            $cachedId .= "_excludeField";
        }

        if($formBuilder){
            $cachedId .= "_formBuilder";
        } else {
            $cachedId .= "_formBuilder_false";
        }

        if($clonedFields){
            $cachedId .= "_clonedFields";
        } else {
            $cachedId .= "_clonedFields_false";
        }
        ...
        $groupQuery = "
            SELECT 
                ...
                g.priority,
                g.context
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_GROUP)."` g
            LEFT JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_GROUP_BELONG)."` b ON b.group_id = g.id
            WHERE 1 = 1
        ";
        ...
        $groups = ACPT_DB::getResults($groupQuery, $groupQueryArgs);
        $groupModels = [];
        ...
        foreach ($groups as $group){
            ...
            $belongs = ACPT_DB::getResults($belongQuery, $belongQueryArgs);
        ...
        foreach ($groupModels as $groupModel){
            if($lazy === false){
                ...
                $boxes = ACPT_DB::getResults($boxesQuery, $boxesQueryArgs);
                ...
                foreach ($boxes as $box){
                    ...
                    if(!empty($box->settings)){
                        $boxModel->setSettings(json_decode($box->settings, true));
                    }
                    ...
                    $fields = ACPT_DB::getResults($sql, $sqlArgs);
                    ...
                    foreach ($fields as $fieldIndex => $field){
                        ...
                        if($formBuilder){
                            if($fieldModel->isForFormBuilder()){
                        ...
                        // add cloned fields
                        if($clonedFields){
                            if($fieldModel->getType() === MetaFieldModel::CLONE_TYPE){
                                foreach ($fieldModel->getClonedFields() as $clonedField){
                                    $boxModel->addField($clonedField);
                                }
                            }
                        }
                        ...
                        $visibilityConditions = ACPT_DB::getResults("
                            SELECT
                                ...
                                front_end,
                                sort
                            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX_VISIBILITY)."`
                            WHERE meta_box_id = %s
                            ORDER BY sort
                        ;", [$boxModel->getId()]);
                        ...
                        foreach ($visibilityConditions as $visibilityCondition){
                            try {
                                ...
                                $visibilityConditionModel = MetaBoxVisibilityModel::hydrateFromArray([
                                ...
                                $boxModel->addVisibilityCondition($visibilityConditionModel);
                            } catch (\Exception $exception){
                                do_action("acpt/error", $exception);
                            }
                        }
    ...
    public static function getBoxNames()
    {
        ...
        $query = "
            ...
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b
	    ";
     ...
     * @return string[]
     */
    public static function getFieldDBNames()
    {
        $records = [];
        $query = "
	        SELECT 
            ...
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` f
            JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b ON f.meta_box_id = b.id
	...
	public static function getNames(): array
	{
		...
		$blockQuery = "
            ...
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BLOCK)."` b
	    ";

		$groups = ACPT_DB::getResults($groupQuery, []);
		foreach ($groups as $group){
			$names['groups'][] = [
				'id' => $group->id,
				'name' => $group->name,
		...
		$boxes = ACPT_DB::getResults($boxQuery, []);
		foreach ($boxes as $box){
			$names['boxes'][] = [
				'id' => $box->id,
				'name' => $box->name,
		...
		$fields  = ACPT_DB::getResults($fieldQuery, []);
		...
		$blocks = ACPT_DB::getResults($blockQuery, []);
		foreach ($blocks as $block){
			$names['blocks'][] = [
				'id' => $block->id,
				'name' => $block->name,
	...
	public static function hydrateMetaBoxFieldModel($field, MetaBoxModel $boxModel, $excludeField = null, $belongsTo = null, $find = null, $clonedFields = false): ?MetaFieldModel
    {
	    ...
	    // Permissions
	    $permissions = PermissionRepository::getByEntityId($fieldModel->getId());
	    foreach ($permissions as $permission){
		    $fieldModel->addPermission($permission);
	    }
	    ...
	    $blocks = ACPT_DB::getResults("
            SELECT
                ...
                sort
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BLOCK)."`
            WHERE meta_field_id = %s 
            ORDER BY sort
        ;", [$field->id]);
		...
		foreach ($blocks as $block){
	        ...
	        ;", $blockChildrenFieldsArgs);
				...
				foreach ($blockChildrenFields as $child){
                    ...
                    if($clonedFields){
                        if($childFieldModel->getType() === MetaFieldModel::CLONE_TYPE){
                            foreach ($childFieldModel->getClonedFields() as $clonedField){
                                $blockModel->addField($clonedField);
                            }
                        }
                    }
				}
		...
		foreach ($children as $child){
            ...
            if($clonedFields){
                if($childFieldModel->getType() === MetaFieldModel::CLONE_TYPE){
                    foreach ($childFieldModel->getClonedFields() as $clonedField){
                        $fieldModel->addChild($clonedField);
                    }
                }
            }
		...
		foreach ($visibilityConditions as $visibilityCondition){
			try {
				...
				if($type['type'] === 'OTHER_FIELDS' and Uuid::isV4Valid($type['value'])){
	...
	public static function getMetaBoxByName($boxName)
	{
		...
		$boxes = ACPT_DB::getResults($boxesQuery, [$boxName]);
        ...
        self::saveInCache($cachedId, $boxModel);
	...
	public static function getMetaFieldByName(array $args): ?MetaFieldModel
	{
        ...
        if(!empty($forgedBy)){
            $forgedByBoxName = $forgedBy['box_name'] ?? $forgedBy['boxName'] ?? '';
            $forgedByFieldName = $forgedBy['field_name'] ?? $forgedBy['fieldName'] ?? '';
            $cachedId .= "_".$forgedByBoxName."_".$forgedByFieldName;
        }
		...
		$fields = ACPT_DB::getResults($sql, [$fieldName, $boxName]);

		if(empty($fields)){

		    // cloned fields
		    if(!empty($forgedBy)){
                $cloneField = MetaRepository::getMetaFieldByName([
                    'boxName' => $forgedBy['box_name'] ?? $forgedBy['boxName'] ?? '',
                    'fieldName' => $forgedBy['field_name'] ?? $forgedBy['fieldName'] ?? ''
                ]);
                ...
                $filteredClonedFields = array_filter($cloneField->getClonedFields(true), function (MetaFieldModel $clonedField) use($fieldName) {
                    return $clonedField->getName() === $fieldName;
                });

                if(Arrays::count($filteredClonedFields) !== 1){
                    return null;
                }

                $fieldModel = array_values($filteredClonedFields)[0];

                if($fieldModel === null){
                    return null;
                }

                $fieldModel->forgeBy($cloneField);

                self::saveInCache($cachedId, $fieldModel);
	    ...
	    if(!empty($boxes)){
		    foreach ($boxes as $box){
            	...
            	";
				    ...
				    $belongs = ACPT_DB::getResults($belongQuery, [$group[0]->id]);
	...
	public static function getMetaBlockByName($name, $lazy = false)
	{
		...
		$blocks = ACPT_DB::getResults($sql, [$name]);
        ...
        try {
	        foreach ($fields as $fieldIndex => $field) {
		        ...
		        if($lazy){
                    ...
                    self::saveInCache($cachedId, $metaField);
		        ...
		        $sql .= " ORDER BY sort;";
		        $boxFields = ACPT_DB::getResults($sql, [$boxModel->getId()]);
	...
	public static function getMetaFields(array $args)
	{
		...
		$sql .= " LEFT JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b ON b.id = f.meta_box_id";
	    $sql .= " WHERE 1 = 1";
    ...
    public static function saveMetaBox(MetaBoxModel $metaBoxModel)
    {
        ...
        try {
	        ...
	        $sql = "
                INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` 
                (
                    `id`,
                    `group_id`,
                    `meta_box_name`,
	        ...
	        ACPT_DB::executeQueryOrThrowException($sql, [
		        ...
		        json_encode($metaBoxModel->getSettings()),
            ...
            foreach ($metaBoxModel->getVisibilityConditions() as $visibilityCondition){
                $sql = "
                    INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX_VISIBILITY)."` 
                        (`id`,
                        `meta_box_id` ,
                        `visibility_type` ,
                        `operator` ,
	...
	public static function saveMetaBoxField(MetaFieldModel $fieldModel)
	{
		...
		try {
			...
			ACPT_DB::executeQueryOrThrowException("DELETE FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_PERMISSION)."` WHERE entity_id = %s;", [$fieldModel->getId()]);
			...
			foreach ($fieldModel->getValidationRules() as $ruleModel){
			    ...
			    $sql = "
			        INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_VALIDATION_RULE_FIELD_PIVOT)."` 
			        (
			            `field_id`,
                        `rule_id`
			        ) VALUES (
		    ...
		    foreach ($fieldModel->getAdvancedOptions() as $advancedOptionModel){
			    $sql = "
                    INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_ADVANCED_OPTION)."` 
                    (`id`,
                    `meta_box_id` ,
                    `meta_field_id` ,
                    `option_key` ,
			...
			foreach ($fieldModel->getOptions() as $optionModel){
				$sql = "
                    INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_OPTION)."` 
                    (`id`,
                    `meta_box_id` ,
                    `meta_field_id` ,
                    `option_label` ,
			...
			foreach ($fieldModel->getVisibilityConditions() as $visibilityCondition){
				$sql = "
                    INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_VISIBILITY)."` 
                    (`id`,
                    `meta_box_id` ,
                    `meta_field_id` ,
                    `visibility_type` ,
			...
			foreach ($fieldModel->getRelations() as $relationModel){
				...
				$sql = "
                    INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_RELATION)."`
                    (
                        `id`,
                        `meta_box_id`,
                        `meta_field_id`,
				...
				if($relationModel->getInversedBy() !== null){
					...
					$sql = "UPDATE `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."`
                        SET `field_type` = %s
                        WHERE id = %s
                    ;";
			...
			if($fieldModel->getType() !== MetaFieldModel::POST_TYPE or ($fieldModel->getRelations()[0] !== null and !$fieldModel->getRelations()[0]->isBidirectional())){
				$query = "
                    SELECT *
                    FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` f
                    JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_RELATION)."` r ON r.meta_field_id = f.id
				...
				foreach ($results as $result){
					$sql = "UPDATE `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_RELATION)."`
                        SET
	...
	{
        if(empty($fieldModel->getName())){
            return;
        }

        if(empty($fieldModel->getType())){
            return;
        }
		...
		$sql = "INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` ( `id`,";

		foreach ($data['fields'] as $index => $field){
			$sql .= '`'.$field.'`';

			if($index < (Arrays::count($data['fields'])-1)){
				$sql .= ',';
			}
		}
	...
	public static function saveMetaBlock(MetaFieldBlockModel $blockModel)
	{
		...
		try {
			$sql = "
            INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BLOCK)."` 
            (   `id`,
	            `meta_box_id` ,
	            `meta_field_id` ,
	            `block_name` ,
	...
	public static function removeMetaOrphans(array $args)
	{
        ...
        $belongs = ACPT_DB::getResults($belongQuery, [$groupId]);
	    ...
	    $deleteBelongsQuery = "
	        DELETE b
	        FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_GROUP_BELONG)."` b
	        WHERE b.group_id = %s
	    ";
	    ...
	    ACPT_DB::executeQueryOrThrowException($deleteBelongsQuery, [
		    $groupId,
	    ]);
	    ...
	    ACPT_DB::executeQueryOrThrowException($deleteFieldsQuery, [
		    $groupId,
	    ]);
	    ...
	    $deleteOptionsQuery = "
	    	DELETE o
			FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_OPTION)."` o
			LEFT JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b on b.id=o.meta_box_id
			WHERE b.group_id = %s
	    ";
	    ...
	    ACPT_DB::executeQueryOrThrowException($deleteOptionsQuery, [
		    $groupId,
	    ]);
	    ...
	    $deleteRelationsQuery = "
	        DELETE r
			FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_RELATION)."` r
			LEFT JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b on b.id=r.meta_box_id
			WHERE b.group_id = %s
	    ";
	    ...
	    ACPT_DB::executeQueryOrThrowException($deleteRelationsQuery, [
		    $groupId,
	    ]);

        // Delete box visibility conditions
        $deleteBoxVisibilityConditionsQuery = "
	    	DELETE v
			FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX_VISIBILITY)."` v
			LEFT JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b on b.id=v.meta_box_id
			WHERE b.group_id = %s
	    ";

        if(isset($ids['boxVisibilityConditions']) and !empty($ids['boxVisibilityConditions'])){
            $deleteBoxVisibilityConditionsQuery .= " AND v.id NOT IN ('".implode("','",$ids['boxVisibilityConditions'])."')";
        }

        $deleteBoxVisibilityConditionsQuery .= ";";

        ACPT_DB::executeQueryOrThrowException($deleteBoxVisibilityConditionsQuery, [
                $groupId,
        ]);

	    // Delete field visibility conditions
	    $deleteVisibilityConditionsQuery = "
	    	DELETE v
			FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_VISIBILITY)."` v
			LEFT JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b on b.id=v.meta_box_id
			WHERE b.group_id = %s
	    ";
	    ...
	    ACPT_DB::executeQueryOrThrowException($deleteVisibilityConditionsQuery, [
		    $groupId,
	    ]);
		...
		$deleteValidationRulesQuery = "
	    	DELETE r
			FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_VALIDATION_RULE_FIELD_PIVOT)."` r
			WHERE 1=1
	    ";
	    ...
	    $deleteBlocksQuery = "
	    	DELETE bl
			FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BLOCK)."` bl
			LEFT JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b on b.id=bl.meta_box_id
			WHERE b.group_id = %s
	    ";
	    ...
	    ACPT_DB::executeQueryOrThrowException($deleteBlocksQuery, [
		    $groupId,
	    ]);
	    ...
	    ACPT_DB::executeQueryOrThrowException($deleteBoxesQuery, [
		    $groupId,
	    ]);
	...
	{
        $select = "SELECT f.id, b.id from `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` f 
            LEFT JOIN  `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b on b.id = f.meta_box_id
            LEFT JOIN  `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_GROUP)."` g ON g.id = b.group_id 
            WHERE g.id IS NULL;";

        $results = ACPT_DB::getResults($select);

        if(!empty($results)){
            $delete = "DELETE f, b from `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` f 
                LEFT JOIN  `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_BOX)."` b on b.id = f.meta_box_id
                LEFT JOIN  `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_GROUP)."` g ON g.id = b.group_id 
                WHERE g.id IS NULL;";

            ACPT_DB::executeQueryOrThrowException($delete, []);
        }
	}
    ...
    public static function removeOrphanVisibilityConditions()
    {
        ...
        foreach ($results as $result){
            ...
            $checkBox = ACPT_DB::getResults($checkBoxQuery, [$result->meta_box_id]);


            if(Arrays::count($checkBox) === 0){
                ACPT_DB::executeQueryOrThrowException($deleteQuery, [$result->id]);
            }

            $checkCustomPostTypeField = ACPT_DB::getResults($checkFieldQuery, [$result->meta_field_id]);
            $checkTaxonomyField = ACPT_DB::getResults($checkTaxonomyFieldQuery, [$result->meta_field_id]);
            if(Arrays::count($checkCustomPostTypeField) === 0 and Arrays::count($checkTaxonomyField) === 0){
                ACPT_DB::executeQueryOrThrowException($deleteQuery, [$result->id]);
            }
            ...
            if($visibilityTypeEnum === 'OTHER_FIELDS'){
                ...
                $checkCustomPostTypeField = ACPT_DB::getResults($checkFieldQuery, [$idToDelete]);
	...
	public static function removeOrphanRelationships()
	{
		...
		if(Arrays::count($results) > 0) {
			foreach ( $results as $result ) {
				...
				if ( Arrays::count( $subResults ) === 0 ) {
					$sql = "DELETE FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_RELATION)."` WHERE meta_field_id = %s;";
					ACPT_DB::executeQueryOrThrowException( $sql, [
						$result->id
					] );

					$sql = "UPDATE `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` SET `field_type` = %s WHERE id = %s;";
					ACPT_DB::executeQueryOrThrowException( $sql, [
		...
		$query = "
            SELECT r.id
            FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_RELATION)."` r
            JOIN `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_FIELD)."` f ON f.id = r.meta_field_id 
            WHERE f.`field_type` != %s
        ";
		...
		if(Arrays::count($results) > 0) {
			foreach ( $results as $result ) {
				$sql = "DELETE FROM `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_META_RELATION)."` WHERE id = %s;";
                ACPT_DB::executeQueryOrThrowException( $sql, [
                    $result->id
                ] );
            }
