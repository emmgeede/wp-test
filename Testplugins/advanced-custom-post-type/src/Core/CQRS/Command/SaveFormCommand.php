...
class SaveFormCommand implements CommandInterface, LogFormatInterface
{
	/**
	 * @var array
	 */
	...
	public function execute(): string
	{
        ...
        $formNames = FormRepository::getNames();
        ...
        // form name
        if(isset($data['id'])){
            $formNames = array_filter($formNames, function ($item) use ($data) {
                return $item !== $data['name'];
            });
        }

        $name = Strings::getTheFirstAvailableName($data['name'], $formNames);
		...
		do_action("acpt/form/save", $this, $formModel);
