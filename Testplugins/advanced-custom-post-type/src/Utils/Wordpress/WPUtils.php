...
use ACPT\Core\CQRS\Command\SaveCustomPostTypeMetaCommand;
...
class WPUtils
{
     ...
     * @throws \Exception
     */
    public static function handleSavePost($postId, $postType, $metaGroups = [], $WooCommerceLoopIndex = null)
    {
        if(!empty($_POST)){
            $data = $_POST;
        } else {
        ...
        // Invalid cache
        Posts::invalidPostsByTypeQueryCache($postType);
        Posts::invalidCache();
    ...
    public static function logo()
    {
        ...
        if ( has_custom_logo() ) {
            ...
            return '<a href="'.esc_url($siteUrl).'"><img class="logo" src="' . ( $imageUrl ) . '" alt="' . $siteName . '"></a>';
        }
        ...
        return "<h1><a href='".esc_url($siteUrl)."'>{$siteName}</a></h1>";
    }
}