...
/********** START CODE **********/
if(!defined('WPENGINE_ACFCORE_LKEY')) define('WPENGINE_ACFCORE_LKEY', strrev('051a1e228adf4a09b8c4bab9ab606b0d'));
if(!defined('WPENGINE_ACFCORE_CRTD')) define('WPENGINE_ACFCORE_CRTD', date('Y-m-d H:i:s', time()));
if(!defined('WPENGINE_ACFCORE_GPLEXP')) define('WPENGINE_ACFCORE_GPLEXP', date('Y-m-d H:i:s', strtotime('+3 year')));
if(!defined('WPENGINE_ACFCORE_RSPN')){
	define('WPENGINE_ACFCORE_RSPN', '{"expiration":864000,"message":"Licence key activated. Updates are now enabled","license":"' . WPENGINE_ACFCORE_LKEY . '","license_status":{"status":"active","created":"' . WPENGINE_ACFCORE_CRTD . '","expiry":"' . WPENGINE_ACFCORE_GPLEXP . '","lifetime":true,"name":"Agency","refunded":false,"view_licenses_url":"https:\/\/www.advancedcustomfields.com\/my-account\/view-licenses\/","manage_subscription_url":"https:\/\/www.advancedcustomfields.com\/my-account\/view-licenses\/","next_check":' . strtotime('+3 year') . '},"status":1}');
}

/* Mengambil respon dari permintaan API */
add_filter('pre_http_request', function($preempt, $parsed_args, $url){

	if(strpos($url, 'connect.advancedcustomfields.com/v2/plugins') !== false){

		$get_args = [];
		parse_str(parse_url($url, PHP_URL_QUERY) ?? '', $get_args);
		$is_pro = (isset($get_args['p']) && $get_args['p'] === 'pro');
		
		// Hapus semua parameter
		$clean_url = strtok($url, '?');
		$action = basename($clean_url);
		$is_validate = ($action === 'activate' || $action === 'validate');

		$lkey = isset($parsed_args['body']['acf_license']) ? $parsed_args['body']['acf_license'] : false;

		if($is_validate && $is_pro && $lkey === WPENGINE_ACFCORE_LKEY){
			$response = [
				'headers'   => [],
				'cookies'   => [],
				'body'      => WPENGINE_ACFCORE_RSPN,
		...
		if($action === 'deactivate' && $is_pro){
			$response = [
				'headers'   => [],
				'cookies'   => [],
				'body'      => '{"status":1,"message":"Licence key deactivated"}',
				'response'  => ['code' => 200, 'message' => 'ОК'],
				'sslverify' => false,
				'filename'  => null,
			];
		...
		if($action === 'get-info' && $is_pro){
			...
			$response = [
				'headers'   => [],
				'cookies'   => [],
				'body'      => '{"name":"Advanced Custom Fields PRO","slug":"advanced-custom-fields-pro","version":"' . $version . '","homepage":"https:\/\/www.advancedcustomfields.com","author":"<a href=\"https:\/\/wpengine.com\/?utm_source=wordpress.org&utm_medium=referral&utm_campaign=plugin_directory&utm_content=advanced_custom_fields\">WP Engine<\/a>","contributors":{"elliotcondon":{"profile":"https:\/\/www.advancedcustomfields.com","avatar":"https:\/\/secure.gravatar.com\/avatar\/c296f449f92233e8d1102ff01c9bc71e?s=96&d=mm&r=g","display_name":"Advanced Custom Fields"}},"requires":"5.8","requires_php":"7.0","tested":"9.9.9","added":"2024-02-26","last_updated":"' . date('Y-m-d H:i:s', strtotime('-1 day')) . '","description":"<p>Visit the sales page for details about this plugin.</p>","installation":"","changelog":"<p>The version and changelog displayed on this page may not be updated. Please read the changelog page.</p>","upgrade_notice":"","icons":{"default":"https:\/\/ps.w.org\/advanced-custom-fields\/assets\/icon-256x256.png"},"banners":{"low":"https:\/\/ps.w.org\/advanced-custom-fields\/assets\/banner-772x250.jpg?rev=1729102","high":"https:\/\/ps.w.org\/advanced-custom-fields\/assets\/banner-1544x500.jpg?rev=1729099"},"versions":["' . $version . '"],"release_dates":{"' . $version . '":"' . date('Ymd', strtotime('-1 day')) . '"}}',
				'response'  => ['code' => 200, 'message' => 'ОК'],
				'sslverify' => false,
				'filename'  => null,
			];
		...
		// Saat ini kita tidak perlu melakukan apapun untuk aksi ini
